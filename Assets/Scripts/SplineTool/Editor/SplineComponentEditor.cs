...
public class SplineComponentEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Space(5);
        GUILayout.BeginHorizontal();
        //GUILayout.BeginArea(new Rect((Screen.width / 2) - 30, 100, 100,100));//(Screen.width / 2) - 100, 0, 200, 200)
        GUILayout.FlexibleSpace();
        Texture _editIcon = AssetDatabase.GetCachedIcon("Assets/Scripts/SplineTool/Editor/Sources/Edit_Icon.png");
        if (GUILayout.Button(_editIcon, GUILayout.Width(40.0f), GUILayout.Height(20.0f)))
        {
            //TODO: Make disabling/enabling possible
        }
        GUILayout.FlexibleSpace();
        GUILayout.EndHorizontal();
    }
    ...
    void HandleCommands(Vector3 wp)
    {
        if (Event.current.type == EventType.ExecuteCommand)
        {
            ...
            {
                SceneView.currentDrawingSceneView.Frame(new Bounds(wp, Vector3.one * 1), false);
                Event.current.Use();
            }
        }
    ...
    [MenuItem("GameObject/Effects/Spline Line")]
    private static void CreateSpline()
    {
        GameObject SplineObject = (GameObject)AssetDatabase.LoadAssetAtPath("Assets/Scripts/SplineTool/Editor/Sources/Spline.prefab", typeof(GameObject));
        Vector3 screenpos = SceneView.lastActiveSceneView.camera.transform.position + SceneView.lastActiveSceneView.camera.transform.forward * 3;
        GameObject cloneSpline = Instantiate(SplineObject, screenpos, Quaternion.identity) as GameObject;
